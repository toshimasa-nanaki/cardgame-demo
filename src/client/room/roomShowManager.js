 ...
 * ルーム一覧用のカード表示を作成する
 */
export const createRoomCardList = (roomList) => {
  Object.keys(roomList).forEach(key => {
    ...
    pCardRoomSetNum.text(
      "セット数：" +
        roomList[key].setNum +
        "セット(" +
        roomList[key].setNum * 4 +
        "ゲーム)"
    );
    pCardRoomRule.text(
      "追加ルール：" + transRuleArrayToString(roomList[key].ruleSet)
    );
      ...
      case "urgentRecruiting":
        ...
        buttonJoinRoom = $("<button>緊急参加(再接続)</button>")
          ...
          .on("click", e => {
            ...
            document.getElementById("retryConnectRoomButton").dataset.roomId =
              roomList[key].roomId;
            socketConnection.emit("join", {
              roomId: roomList[key].roomId,
              playerName: $("#playerName").val()
