...
module.exports.createRoom = (entryRoomInfo, socketObj) => {
  ...
  roomObj["roomDispName"] =
    entryRoomInfo.dispName === ""
      ? createDefaultRoomName()
      : commonUtil.htmlentities(entryRoomInfo.dispName);
  roomObj["capacity"] = Number(entryRoomInfo.capacity);
  roomObj["setNum"] = Number(entryRoomInfo.setNum);
  roomObj["ruleSet"] = entryRoomInfo.ruleSet;
...
module.exports.reJoinRoom = (reJoinInfo, socketObj) => {
  ...
  let reconnectUser = {
    dispName:
      reJoinInfo.playerName !== ""
        ? commonUtil.htmlentities(reJoinInfo.playerName)
        : storeData.persistentData[reJoinInfo.roomId]["users"][
            reJoinInfo.reconnectUserId
          ].dispName,
    card:
  ...
  LOGGER.debug(
    "今のユーザー情報:" +
      JSON.stringify(storeData.persistentData[reJoinInfo.roomId]["users"])
  );
  storeData.persistentData[reJoinInfo.roomId]["users"][
    socketObj.id
  ] = reconnectUser;
  ...
  delete storeData.persistentData[reJoinInfo.roomId]["users"][
    reJoinInfo.reconnectUserId
  ];
  ...
  LOGGER.debug(
    "抜く前のleaveUserIds:" +
      JSON.stringify(storeData.persistentData[reJoinInfo.roomId].leaveUserIds)
  );
  ...
  storeData.persistentData[reJoinInfo.roomId].leaveUserIds.some(ele => {
    ...
    if (
      deleteLeaveUserPos ===
      storeData.persistentData[reJoinInfo.roomId].leaveUserIds.length
    ) {
      //最後まで見つからなかったということ
  ...
  if (deleteLeaveUserPos !== -1) {
    storeData.persistentData[reJoinInfo.roomId].leaveUserIds.splice(
      deleteLeaveUserPos,
      1
    );
  } else {
  ...
  if (storeData.persistentData[reJoinInfo.roomId].giveCardPhase) {
    let pos =
      storeData.persistentData[reJoinInfo.roomId]["order"].indexOf(
        socketObj.id
      ) + 1;
    //カード譲渡のフェーズに戻る必要がある。
  ...
  io.to(socketObj.id).emit("reJoinOK", {
    ...
    card:
      storeData.persistentData[reJoinInfo.roomId]["users"][socketObj.id].card,
    yourTurn:
      storeData.persistentData[reJoinInfo.roomId].currentTurnPos ===
      storeData.persistentData[reJoinInfo.roomId].order.indexOf(socketObj.id)
        ? true
        : false,
    playerName:
      storeData.persistentData[reJoinInfo.roomId]["users"][
    ...
    playerName2:
      storeData.persistentData[reJoinInfo.roomId]["users"][socketObj.id]
        .dispName,
    playerPoint:
      storeData.persistentData[reJoinInfo.roomId]["users"][socketObj.id].point,
    ...
    roomInfo: {
      ...
      rankingHistory:
        storeData.persistentData[reJoinInfo.roomId].rankingHistory, //ランキングヒストリー
  ...
  for (let [key, value] of Object.entries(
    ...
    if (key !== socketObj.id)
      io.to(key).emit("otherMemberReJoinedOK", {
        playerName: reconnectUser.dispName,
        memberOK:
          storeData.persistentData[reJoinInfo.roomId].leaveUserIds.length === 0
      });
  }
